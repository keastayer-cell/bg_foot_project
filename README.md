...
- `dev` — основная и default-ветка текущей разработки; push запускает CI, но не обновляет сервер;
- `test` — версия тестового окружения; push запускает автоматический деплой на тестовый VPS;
- `prod` — точная история production-релизов; автоматический production-деплой пока не настроен.

Рабочий поток: `dev -> test -> prod`. Все изменения сначала фиксируются в `dev`, затем целиком проверяются в `test` и только после приёмки переносятся в `prod`. Напрямую разрабатывать в `test` и `prod` нельзя. Ветка `main` сознательно не используется: в этой модели её назначение полностью дублировало бы `prod`.
...
После проверки тестового релиза тот же commit переносится из `test` в `prod` без дополнительных функциональных изменений. Выпущенная версия отмечается Git-тегом вида `v1.0.0`.

Автоматического production workflow в репозитории пока нет: push в `prod` сам по себе сервер не обновляет. Общие backup/restore, ротация логов, smoke-check и процедура rollback описаны в [эксплуатационной документации](docs/operations.md). Production restore требует отдельного подтверждения и выполняется только во время согласованного downtime.
