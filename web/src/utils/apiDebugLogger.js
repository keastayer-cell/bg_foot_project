...
function isApiRequest(url, apiBaseUrl) {
  try {
...
export function installApiDebugLogger({ apiBaseUrl, enabled }) {
  if (!enabled || window.__footballStatsApiDebugLoggerInstalled) {
    return
  }

  const originalFetch = window.fetch.bind(window)
  window.__footballStatsApiDebugLoggerInstalled = true
