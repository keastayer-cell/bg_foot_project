...
- Реализован первый реальный слой протокола матча без фронтовых моков:
  - таблица `work.w_match_protocol` хранит статус матча, счет, лучшего игрока, заметки и временные метки
  - таблица `work.w_match_event` хранит события матча: голы, карточки, замены и другие отметки протокола
  - для уже существующих матчей протоколы backfill-ятся миграцией `V22`
- Добавлен публичный endpoint `GET /api/matches/{matchId}`:
  - возвращает матч, команды, сезон, тур, счет/статус и список событий протокола
- Добавлен endpoint `PUT /api/matches/{matchId}/protocol`:
  - сохраняет реальный протокол матча в БД
  - может пересчитывать счет из списка событий, если score не передан явно
...
- `football-stats-web/src/pages/Match.vue` переведен с локального fake-store на реальный `GET /api/matches/{id}`
- Составы команд на странице матча пока не реализованы в БД и UI:
  - следующим этапом нужны отдельные сущности матчевой заявки/состава (`match_team_sheet`, `match_player_entry`)
